...
SECTORS = {
    ...
    "التامين":        ["8010","8020","8030","8040","8050","8060","8070","8100",
                      ...
                      "8311","8320","8330","8340"],
    "الاستثمار":      ["1111","4280","4281","4290","4291","4349","4330","4331",
...
def load_all_stocks_from_intel():
    """
    يقرأ جميع الأسهم من market_intel.json
    market_intelligence.py حلّل 220+ سهم وحفظهم هنا
    نقرأ جميعهم — لا أحد يُستبعد
    """
    try:
...
def enrich_top10_with_live_data(ranked):
    """
    بعد ترتيب الأسهم بالـ Score
    نحدّث أفضل 10 فقط ببيانات حية من API
    هذا يضمن دقة الأسعار والأهداف للإشارة النهائية
    """
    print(f"\n  تحديث أفضل 10 بيانات حية...")
    updated = 0
    ...
    print(f"  تم تحديث {updated} سهم ببيانات حية ✅")
    return ranked


def fallback_from_api():
    """
    إذا فشل market_intel.json نجلب من API مباشرة
    """
...
def main():
    ...
    # الخطوة 1 — market_intelligence يحلل 220+ سهم ويحفظ market_intel.json
    top_sectors = []
    try:
        import os as _os
        ...
        intel       = run_intel() or {}
        top_sectors = intel.get("top_sectors", [])
        print(f"\n Market Intelligence: {len(intel.get('top_stocks',[]))} سهم | Sectors: {', '.join(top_sectors)}")
    except Exception as e:
        print(f"\n Market Intelligence error: {e}")

    # الخطوة 2 — قراءة جميع الأسهم من market_intel.json (بدون جلب من API مرة ثانية)
    stocks, intel_sectors = load_all_stocks_from_intel()
    if intel_sectors:
        top_sectors = intel_sectors

    # fallback إذا فشل market_intel
    if len(stocks) < 5:
    ...
    # الخطوة 3 — تقييم جميع الأسهم بالـ Score الكامل
    ranked = []
    for stock in stocks:
    ...
    # الخطوة 4 — تحديث أفضل 10 ببيانات حية دقيقة
