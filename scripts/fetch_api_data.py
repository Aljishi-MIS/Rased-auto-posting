...
def claude_review(candidates, top_sectors):
    ...
    for i, c in enumerate(candidates[:3], 1):
        ...
        summaries.append(
            ...
            f"   R:R: {c['rr']}\n"
            f"   الاخبار: {news.get('sentiment','neutral')} | {news.get('summary','لا يوجد')}\n"
            f"   تاثير الاخبار على Score: {news.get('score_delta',0):+d}"
        )
...
السياق العام:
- القطاعات المتصدرة اليوم: {', '.join(top_sectors) if top_sectors else 'غير محدد'}
- عدد المرشحين: {len(candidates)}

هذه افضل الاسهم المرشحة مع جميع المعطيات الفنية والاخبارية:
...
اجب بـ JSON فقط:
{{
  "selected_index": رقم 1 او 2 او 3 او 0 للرفض,
  "decision": "publish" او "reject",
  "symbol": "رمز السهم او فارغ عند الرفض",
  "reason": "جملة واحدة سبب القرار بالعربي",
  "confidence": "عالية" او "متوسطة" او "منخفضة",
  "warning": "تحذير ان وجد او اتركه فارغ",
  "note": "ملاحظة تحليلية للمشتركين"
}}
