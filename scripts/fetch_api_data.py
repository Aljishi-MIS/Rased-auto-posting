...
def calc_targets(price, high, low, resistance, support, change_pct):
    """
    T1 = +5% من الدخول
    T2 = +10% من الدخول (ثابت — للوصول خلال 7-10 أيام)
    R:R يُحسب على T2
    """
    ...
    # وقف خسارة ديناميكي لا يتجاوز -4%
...
def enrich_top10_with_historical(ranked):
    ...
    for r in ranked[:10]:
        ...
        # تحديث avg_volume بدقة من 20 يوم تاريخي
        if len(volumes) >= 20:
        ...
        print(f"    {stock.get('name','')[:18]:<18} OBV:{obv_pts:>3} "
              f"MACD:{macd_pts:>3} 52W:{w52_pts:>3} Accel:{accel:>3} {icon}{bonus:+d} → {r['score']}")
        enriched += 1
...
def acceleration_score(closes, volumes):
    ...
    if len(closes) < 10 or len(volumes) < 10:
        return 0
    price_accel = (closes[-1] - closes[-6]) / closes[-6] * 100 if closes[-6] > 0 else 0
    price_prev  = (closes[-6] - closes[-11]) / closes[-11] * 100 if len(closes) >= 11 and closes[-11] > 0 else 0
