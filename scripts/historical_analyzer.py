...
# ✅ إصلاح: الإشارة الذهبية تعمل فقط خلال ساعة قبل السوق أو أثناءه
GOLDEN_EARLIEST_HOUR = 8   # 8:00 KSA
GOLDEN_LATEST_HOUR   = 15  # 3:00 KSA
...
def main():
    ...
    now = datetime.now(KSA)
    print(f"  الوقت: {now.strftime('%H:%M')} KSA | اليوم: {now.weekday()}")

    # ✅ إصلاح: تحقق من النافذة الزمنية — لا نشر قبل 8 صباحاً أو بعد 3 عصراً
    hour = now.hour
    weekday = now.weekday()  # 0=Mon, 6=Sun; السوق: 6,0,1,2,3
    market_days = [6, 0, 1, 2, 3]

    if weekday not in market_days:
        print("  يوم إجازة — تخطي الإشارة الذهبية")
        sys.exit(0)

    if not (GOLDEN_EARLIEST_HOUR <= hour < GOLDEN_LATEST_HOUR):
        print(f"  الوقت {now.strftime('%H:%M')} خارج نافذة الإشارة الذهبية ({GOLDEN_EARLIEST_HOUR}:00–{GOLDEN_LATEST_HOUR}:00)")
        sys.exit(0)

    # تشغيل market_intelligence أولاً
    ...
    # ✅ إصلاح: تحقق من R:R قبل الحفظ
    if signal["rr"] < 1.2:
        print(f"\n  الإشارة الذهبية R:R = {signal['rr']} ضعيف جداً — تخطي")
        sys.exit(1)
